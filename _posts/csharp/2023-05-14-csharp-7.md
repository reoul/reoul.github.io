...
lastmod:    2023-05-15T03:04:00+09:00
---

<!-- description : 25자에서 160자 사이 -->
System.Collections.Generic.List<Type>에 대해 공부하다가 알게된 정보를 정리한 글입니다.<br>
주로 어떤 원리로 작동되는 지 공부해 정리하였습니다.
{: .notice--warning}

## List<Type>란?

- List<Type>는 필요에 따라 크기가 동적으로 증가하는 T타입 배열이다
- **내부에 요소를 보관할 배열을 가지고 있다**
- 정렬을 보장하지 않는다
- 인덱스는 정수 인덱스로 접근이 가능하다

## List<Type>는 어떻게 동적 배열로 작동하는가?
...
```c#
- List(IEnumerable<Type> collection)
```

- collection이 `ICollection<Type>`를 구현한 경우
