...
- 키를 사용하면 빠르게 관련된 값을 찾을 수 있다
- Hash테이블이다
- 내부에 2가지 배열을 가지고 있다
  - int[] buckets : Hash를 사용하고 있는 Entry의 entries의 인덱스를 저장하는 배열
  - Entry[] entries : 데이터인 Entry를 저장하는 배열

## Entry

- Key와 Value를 저장하는 타입
- 데이터를 Entry 타입으로 가지고 있는다
- 내부 데이터
  - key : Key값
  - value : Value값
  - next : 다음 Entry Index
...
- Hash에 대한 buckets 값이 -1이면 Hash가 같은 Entry가 없으므로 false 반환
